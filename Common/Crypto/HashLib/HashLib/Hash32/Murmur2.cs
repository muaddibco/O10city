...
namespace HashLib.Hash32
{
    internal class Murmur2 : MultipleTransformNonBlock, IHash32, IFastHash32, IHashWithKey
    {
        ...
        private int InternalComputeBytes(Span<byte> a_data)
        {
        ...
        protected override HashResult ComputeAggregatedBytes(Span<byte> a_data)
        {
            return new HashResult(InternalComputeBytes(a_data));
        }

        public int ComputeBytesFast(Span<byte> a_data)
        {
            Initialize();
        ...
        {
            return ComputeUShortFast(a_data);
        }
        ...
        public int ComputeULongsFast(ulong[] a_data)
        {
            ...
            while (length >= 8)
            {
                TransformULongFast(unchecked(a_data[current_index++]));
