...
namespace HashLib
{
    internal abstract class MultipleTransformNonBlock : Hash, INonBlockHash
    {
        ...
        public override void TransformBytes(Span<byte> a_data, int a_index, int a_length)
        {
            ...
            m_list.Add(new ArraySegment<byte>(a_data.ToArray(), a_index, a_length));
        }
        ...
        public override HashResult ComputeBytes(Span<byte> a_data)
        {
            Initialize();
            return ComputeAggregatedBytes(a_data);
        }

        protected abstract HashResult ComputeAggregatedBytes(Span<byte> a_data);
    }
}
