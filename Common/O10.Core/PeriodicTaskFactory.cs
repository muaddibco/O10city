...
namespace O10.Core
{
	...
	public static class PeriodicTaskFactory
    {
        ...
        /// bubbled up to the periodic task.
        /// </remarks>
        public static Task Start(Action<object> action,
                                 object state,
                                 int intervalInMilliseconds = Timeout.Infinite,
                                 int delayInMilliseconds = 0,
                                 int duration = Timeout.Infinite,
        ...
        {
            Stopwatch stopWatch = new Stopwatch();
            Action<object> wrapperAction = o =>
            {
                CheckIfCancelled(cancelToken);
                action(o);
            };

            Action<object> mainAction = o =>
            {
                MainPeriodicTaskAction(o, intervalInMilliseconds, delayInMilliseconds, duration, maxIterations, cancelToken, stopWatch, synchronous, wrapperAction, periodicTaskCreationOptions);
            };

            return Task.Factory.StartNew(mainAction, state, cancelToken, TaskCreationOptions.LongRunning, TaskScheduler.Current);
        }
