using System;
using O10.Transactions.Core.DataModel.Stealth;
using O10.Transactions.Core.Enums;
using O10.Transactions.Core.Exceptions;
using O10.Core.Architecture;

using O10.Core.Cryptography;
using O10.Core.Identity;

namespace O10.Transactions.Core.Parsers.Stealth
{
	[RegisterExtension(typeof(IBlockParser), Lifetime = LifetimeManagement.Singleton)]
	public class TransitionCompromisedProofsParser : StealthTransactionParserBase
	{
		public TransitionCompromisedProofsParser(IIdentityKeyProvidersRegistry identityKeyProvidersRegistry) : base(identityKeyProvidersRegistry)
		{
		}

		public override ushort BlockType => ActionTypes.Stealth_TransitionCompromisedProofs;

		protected override Memory<byte> ParseStealthTransaction(ushort version, Memory<byte> spanBody, out StealthTransactionBase StealthBase)
		{
			StealthTransactionBase block = null;

			if (version == 1)
			{
				int readBytes = 0;

				ReadEcdhTupleCA(ref spanBody, ref readBytes, out EcdhTupleCA ecdhTuple);
				ReadSurjectionProof(ref spanBody, ref readBytes, out SurjectionProof ownershipProofs);
				ReadSurjectionProof(ref spanBody, ref readBytes, out SurjectionProof eligibilityProofs);
				ReadCommitment(ref spanBody, ref readBytes, out byte[] compromisedKeyImage);
				// TODO: need to add parsing of compromised Public Keys and Signature

				block = new TransitionCompromisedProofs
				{
					EcdhTuple = ecdhTuple,
					OwnershipProof = ownershipProofs,
					EligibilityProof = eligibilityProofs,
					CompromisedKeyImage = compromisedKeyImage
                };

				StealthBase = block;

				return spanBody.Slice(readBytes);
			}

			throw new BlockVersionNotSupportedException(version, BlockType);
		}
	}
}
