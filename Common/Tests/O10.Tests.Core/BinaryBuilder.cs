...
namespace O10.Tests.Core
{
    public class BinaryHelper
    {
		...
		public static byte[] GetStealthPacket(PacketType packetType, ulong syncBlockHeight, uint nonce, byte[] powHash, ushort version, ushort blockType, byte[] keyImage, byte[] destinationKey, byte[] destinationKey2, byte[] transactionPublicKey, byte[] body, byte[][] pubKeys, byte[] secretKey, int secIndex, out RingSignature[] ringSignatures)
        {
            byte[] bodyBytes = null;
            byte[] result = null;
